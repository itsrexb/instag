...
class ControllerAffiliateRegister extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$affiliate_data = $this->request->post;

			// Check to see if this affiliate came in via an affiliate link
			if (isset($this->request->cookie['tracking'])) {
				$parent_affiliate_info = $this->model_affiliate_affiliate->getAffiliateByCode($this->request->cookie['tracking']);

				if ($parent_affiliate_info) {
					$affiliate_data['parent_id'] = $parent_affiliate_info['affiliate_id'];
				} else {
					$affiliate_data['parent_id'] = 0;
				}
			} else {
				$affiliate_data['parent_id'] = 0;
			}

			$affiliate_id = $this->model_affiliate_affiliate->addAffiliate($affiliate_data);
