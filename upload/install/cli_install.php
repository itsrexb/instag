...
//
// Command line tool for installing instag
...
//                               --http_server http://localhost/instag
//
...
function check_requirements() {
	$error = null;
	if (phpversion() < '5.0') {
		$error = 'Warning: You need to use PHP5 or above for instag to work!';
	}
	...
	if (ini_get('session.auto_start')) {
		$error = 'Warning: instag will not work with session.auto_start enabled!';
	}

	if (!extension_loaded('mysqli')) {
		$error = 'Warning: MySQLi extension needs to be loaded for instag to work!';
	}

	if (!extension_loaded('gd')) {
		$error = 'Warning: GD extension needs to be loaded for instag to work!';
	}

	if (!extension_loaded('curl')) {
		$error = 'Warning: CURL extension needs to be loaded for instag to work!';
	}

	if (!function_exists('mcrypt_encrypt')) {
		$error = 'Warning: mCrypt extension needs to be loaded for instag to work!';
	}

	if (!extension_loaded('zlib')) {
		$error = 'Warning: ZLIB extension needs to be loaded for instag to work!';
	}
...
function write_config_files($options) {
	...
	$output .= '// DIR' . "\n";
	$output .= 'define(\'DIR_APPLICATION\', \'' . DIR_instag . 'catalog/\');' . "\n";
	$output .= 'define(\'DIR_SYSTEM\', \'' . DIR_instag . 'system/\');' . "\n";
	$output .= 'define(\'DIR_DATABASE\', \'' . DIR_instag . 'system/database/\');' . "\n";
	$output .= 'define(\'DIR_LANGUAGE\', \'' . DIR_instag . 'catalog/language/\');' . "\n";
	$output .= 'define(\'DIR_TEMPLATE\', \'' . DIR_instag . 'catalog/view/theme/\');' . "\n";
	$output .= 'define(\'DIR_CONFIG\', \'' . DIR_instag . 'system/config/\');' . "\n";
	$output .= 'define(\'DIR_IMAGE\', \'' . DIR_instag . 'image/\');' . "\n";
	$output .= 'define(\'DIR_CACHE\', \'' . DIR_instag . 'system/storage/cache/\');' . "\n";
	$output .= 'define(\'DIR_DOWNLOAD\', \'' . DIR_instag . 'system/storage/download/\');' . "\n";
	$output .= 'define(\'DIR_UPLOAD\', \'' . DIR_instag . 'system/storage/upload/\');' . "\n";
	$output .= 'define(\'DIR_LOGS\', \'' . DIR_instag . 'system/storage/logs/\');' . "\n\n";
	...
	$output .= '// DIR' . "\n";
	$output .= 'define(\'DIR_APPLICATION\', \'' . DIR_instag . 'admin/\');' . "\n";
	$output .= 'define(\'DIR_SYSTEM\', \'' . DIR_instag . 'system/\');' . "\n";
	$output .= 'define(\'DIR_DATABASE\', \'' . DIR_instag . 'system/database/\');' . "\n";
	$output .= 'define(\'DIR_LANGUAGE\', \'' . DIR_instag . 'admin/language/\');' . "\n";
	$output .= 'define(\'DIR_TEMPLATE\', \'' . DIR_instag . 'admin/view/template/\');' . "\n";
	$output .= 'define(\'DIR_CONFIG\', \'' . DIR_instag . 'system/config/\');' . "\n";
	$output .= 'define(\'DIR_IMAGE\', \'' . DIR_instag . 'image/\');' . "\n";
	$output .= 'define(\'DIR_CACHE\', \'' . DIR_instag . 'system/storage/cache/\');' . "\n";
	$output .= 'define(\'DIR_DOWNLOAD\', \'' . DIR_instag . 'system/storage/download/\');' . "\n";
	$output .= 'define(\'DIR_UPLOAD\', \'' . DIR_instag . 'system/storage/upload/\');' . "\n";
	$output .= 'define(\'DIR_LOGS\', \'' . DIR_instag . 'system/storage/logs/\');' . "\n";
	$output .= 'define(\'DIR_CATALOG\', \'' . DIR_instag . 'catalog/\');' . "\n\n";
...
function dir_permissions() {
	$dirs = array(
		DIR_instag . 'image/',
		DIR_instag . 'system/storage/download/',
		DIR_instag . 'system/storage/upload/',
		DIR_instag . 'system/storage/cache/',
		DIR_instag . 'system/storage/logs/',
	);
...
case "install":
	try {
		...
		define('HTTP_instag', $options['http_server']);
		$valid = valid($options);
		if (!$valid[0]) {
		...
		install($options);
		echo "SUCCESS! instag successfully installed on your server\n";
