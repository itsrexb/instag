...
class ModelUpgrade extends Model {
	public function upgrade() {
		...
		foreach($lines as $line) {
			// Set any prefix
			$line = str_replace("DROP TABLE IF EXISTS `instag_", "DROP TABLE IF EXISTS `" . DB_PREFIX, $line);

			$line = str_replace("CREATE TABLE `instag_", "CREATE TABLE `" . DB_PREFIX, $line);
		...
		foreach ($table_new_data as $table) {
			...
			if (!isset($table_old_data[$table['name']])) {
				...
				foreach ($query->rows as $result) {
					if (!in_array($result['Key_name'], $dropped_indexes)) {
						...
						$dropped_indexes[] = $result['Key_name'];
					}
				}
				...
				foreach ($table['index'] as $index_name => $index) {
					...
					if ($index_data) {
						$this->db->query("ALTER TABLE `" . $table['name'] . "` ADD INDEX `" . $index_name . "` (" . implode(',', $index_data) . ")");
					}
				}
