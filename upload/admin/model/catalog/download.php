...
class ModelCatalogDownload extends Model {
	...
	public function getDownload($download_id) {
		...
		if ($query->row) {
			$download_data = $query->row;

			$store_timezone = new DateTimeZone($this->config->get('config_timezone'));

			$date_added = new DateTime($download_data['date_added']);
			$date_added->setTimezone($store_timezone);

			$download_data['date_added'] = $date_added->format('Y-m-d H:i:s');
		} else {
			$download_data = array();
		}
	...
	public function getDownloads($data = array()) {
		...
		foreach ($query->rows as $download) {
			$date_added = new DateTime($download['date_added']);
			$date_added->setTimezone($store_timezone);

			$download['date_added'] = $date_added->format('Y-m-d H:i:s');
