...
class ControllerReportAffiliateActivity extends Controller {
	public function index() {
		...
		if (isset($this->request->get['filter_affiliate'])) {
			$filter_affiliate = trim($this->request->get['filter_affiliate']);
		} else {
			$filter_affiliate = null;
		}

		if (isset($this->request->get['filter_ip'])) {
			$filter_ip = trim($this->request->get['filter_ip']);
		} else {
			$filter_ip = null;
		}
		...
		foreach ($results as $result) {
			...
			$data['activities'][] = array(
				'comment'    => str_replace('affiliate_id=', $this->url->link('affiliate/affiliate/edit', 'token=' . $this->session->data['token'] . '&affiliate_id=', true), $comment),
				'ip'         => $result['ip'],
