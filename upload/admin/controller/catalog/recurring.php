...
class ControllerCatalogRecurring extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('catalog/recurring', $url, true));
		}
	...
	public function copy() {
		...
		if (isset($this->request->post['selected']) && $this->validateCopy()) {
			...
			$this->session->data['success'] = sprintf($this->language->get('success_copy'), count($this->request->post['selected']));
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('catalog/recurring', $url, true)
		);

		$data['add']    = $this->url->link('catalog/recurring/add', $url, true);
		$data['copy']   = $this->url->link('catalog/recurring/copy', $url, true);
		$data['delete'] = $this->url->link('catalog/recurring/delete', $url, true);
		...
		foreach ($results as $result) {
			$data['recurrings'][] = array(
				...
				'edit'         => $this->url->link('catalog/recurring/edit', $url . '&recurring_id=' . $result['recurring_id'], true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('catalog/recurring', $url . '&sort=pd.name', true);
		$data['sort_sort_order'] = $this->url->link('catalog/recurring', $url . '&sort=p.sort_order', true);
		...
		$pagination->url = $this->url->link('catalog/recurring', $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['recurring_id'])) {
			$data['action'] = $this->url->link('catalog/recurring/add', $url, true);
		} else {
			$data['action'] = $this->url->link('catalog/recurring/edit', $url . '&recurring_id=' . $this->request->get['recurring_id'], true);
		}

		$data['cancel'] = $this->url->link('catalog/recurring', $url, true);
		...
		if (isset($this->request->post['price_type'])) {
			...
			$data['price_type'] = $recurring_info['price_type'];
		} else {
			$data['price_type'] = '';
		}
		...
		if (isset($this->request->post['trial_price_type'])) {
			$data['trial_price_type'] = $this->request->post['trial_price_type'];
		} else if (!empty($recurring_info)) {
			$data['trial_price_type'] = $recurring_info['trial_price_type'];
		} else {
			$data['trial_price_type'] = '';
		}
