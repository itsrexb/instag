...
<div id="content">
  <div class="page-header">
    <div class="container-fluid">
      ...
      <button class="btn btn-info resend-link" data-link="<?php echo $resend_link; ?>" data-toggle="tooltip" title="<?php echo $text_resend; ?>"><i class="fa fa-envelope"></i></button>
      ...
      <div class="col-md-4">
        <div class="panel panel-default">
          ...
          <table class="table">
            ...
            <?php if ($affiliate) { ?>
            <tr>
              <td><button data-toggle="tooltip" title="<?php echo $text_affiliate; ?>" class="btn btn-info btn-xs"><i class="fa fa-sitemap fa-fw"></i></button></td>
              <td><a href="<?php echo $href_affiliate; ?>"><?php echo $affiliate; ?></a></td>
            </tr>
            <?php } ?>
          </table>
        </div>
      ...
      <div class="panel-body">
        <table class="table table-bordered">
          ...
          <tbody>
            ...
            <tr>
              <td class="text-left" colspan="2"><a href="<?php echo $product['href']; ?>"><?php echo $product['name']; ?></a><br>
                <span style="margin-left: 5px;">- <?php echo ucwords($product['account_type'])?>: <a href="<?php echo $product['href_account']; ?>"><?php echo $product['account_username']; ?></a></span></td>
    ...
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        <ul class="nav nav-tabs">
          ...
          <?php if ($permission_access_transaction) { ?>
...
// Login to the API
$.ajax({
	url: '<?php echo $store_api_url; ?>index.php?route=api/login',
	type: 'post',
	dataType: 'json',
...
function addOrderHistory(order_status_id, notify, override, comment) {
	notify   = typeof notify !== 'undefined' ? notify : false;
	override = typeof override !== 'undefined' ? override : false;
	comment  = typeof comment !== 'undefined' ? comment : '';

	$.ajax({
		url: '<?php echo $store_api_url; ?>index.php?route=api/order/history&token=' + token + '&order_id=<?php echo $order_id; ?>',
		type: 'post',
		dataType: 'json',
		data: 'order_status_id=' + encodeURIComponent(order_status_id) + '&notify=' + (notify ? 1 : 0) + '&override=' + (override ? 1 : 0) + '&comment=' + encodeURIComponent(comment),
		beforeSend: function() {
...
$('#button-history').on('click', function() {
	...
	addOrderHistory(
		$('select[name="order_status_id"]').val(),
		$('input[name="notify"]').prop('checked'),
		$('input[name="override"]').prop('checked'),
		$('textarea[name="comment"]').val()
	);
});
...
<?php if ($permission_access_transaction) { ?>
$('#transaction').load('index.php?route=sale/order_transaction&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>');
<?php } ?>
...
$('.resend-link').on('click', function(e) {
	...
	if (confirm('<?php echo $text_confirm; ?>')) {
		$.getJSON(
			$(this).attr('data-link')+'&token='+token,
			function(data) {
				if (data.error == 0) {
					alert('<?php echo $text_resend_success; ?>' + data.order_id);
				} else {
					alert('<?php echo $text_resend_error; ?>' + data.order_id);
				}
			}
		);
	}
